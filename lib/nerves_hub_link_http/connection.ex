defmodule NervesHubLinkHTTP.Connection do
  @moduledoc """
  ...
  In practice, this state is set after a HTTP request fails or succeeds. This makes it
  ...
  defmodule MyApp.Checker do
    def health_check do
      with :ok <- NervesHubLinkHTTP.Connection.check,
  ...
  ```
  # 60 second timeout
  config :nerves_hub_link_http, connection_timeout: 60
  ```
  """
  @spec check() :: :ok | {:error, {:disconnected_too_long, integer()}} | {:error, :no_agent}
  def check() do
    timeout = Application.get_env(:nerves_hub_link_http, :connection_timeout, 900)
    now = current_time()
