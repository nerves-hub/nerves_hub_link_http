...
### Adding NervesHubLinkHTTP to your project

The first step is to add `nerves_hub_link_http` to your target dependencies in your
...
Next, update your `config.exs` so that the `nerves_hub_link_http` library can help
provision devices. Do this by adding `provisioning: :nerves_hub_link_http` to the
...
You can now use `NervesHubLinkHTTP.update/0` to request any available updates from NervesHub.
...
The `nerves_hub_link_http` dependency converts key names to public keys at compile time.
If you haven't compiled your project yet, run `mix firmware` now. If you have
compiled it, `mix` won't know to recompile `nerves_hub_link_http` due to the configuration
